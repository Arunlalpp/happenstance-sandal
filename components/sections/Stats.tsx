...
export function Stats() {
  ...
  return (
    <section ref={root} className="relative border-y border-sand/10 bg-umber/40 px-6 py-24 md:px-10">
      <div className="mx-auto grid max-w-[1600px] gap-10 sm:grid-cols-2 lg:grid-cols-4">
        {STATS.map((s) => (
          <div key={s.label} className="text-center md:text-left">
            ...
            <p className="mt-3 font-mono text-xs uppercase tracking-label text-dune">{s.label}</p>
          </div>
        ))}
      </div>
    </section>
